...
if __name__ == "__main__":
    ...
    for prefix in prefixs:
        ...
        if totalpattern.count('*') < 4:
            ...
            nodes_heapq, pre_target_num, pre_active_num = preScan(
                new_nodes, ipv6, preScan_num=preScan_num, miniBudget=miniBudget, activefile=activefile, targetfile=targetfile)
            ...
            target_num, active_num = scan_feedback2(
                nodes_heapq, ipv6, batch_size=batchsize, epoch=epoch, targetfile=targetfile, activefile=activefile)
            hitrate = (active_num+pre_active_num) / (target_num+pre_target_num)
            end = time.time()
            with open(logfile, 'a+') as f:
                f.write("\n#{},budget:{}\n".format(prefix, budget))
                f.write('target:{},active:{},hitrate:{},duration:{}\n'.format(
                    target_num+pre_target_num, active_num+pre_active_num, hitrate, end-start))
